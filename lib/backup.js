...
function applyRestored(data) {
  ...
  if (!fileExists('tokens.json') && data.tokens && typeof data.tokens === 'object' && data.tokens.refresh_token) {
    store.saveTokens(data.tokens);
  }
...
function startPeriodicBackup(intervalMs = 5 * 60 * 1000) {
  ...
  timer = setInterval(() => {
    ...
    drive.cleanupOldSelections().catch(() => {}); // tri auto : purge des dossiers expirés
    now();
  }, intervalMs);
