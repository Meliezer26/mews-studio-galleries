...
/** 'resend' si une clé API Resend est enregistrée, sinon 'smtp'. */
function provider() {
  return smtp().apiKey ? 'resend' : 'smtp';
}

function isConfigured() {
  const n = smtp();
  return !!(n.enabled && n.from && (n.apiKey || n.host));
}
...
async function sendMail(mailOptions) {
  if (provider() === 'resend') return sendViaResend(mailOptions);
...
async function sendTest() {
  ...
  if (!recipient()) throw new Error('Aucun destinataire défini.');
  const via = provider() === 'resend' ? 'Resend' : 'SMTP';
  await sendMail({
    from: smtp().from,
    to: recipient(),
