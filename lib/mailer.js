...
function buildContent(info) {
  ...
  if (info.eventNames) lines.push('Prénoms (mise en page) : ' + info.eventNames);
  lines.push('');
  rows.forEach((a) => {
  ...
  ${info.clientEmail ? `<p style="margin:0 0 6px;font-size:14px;color:#555">E-mail : <a href="mailto:${escapeHtml(info.clientEmail)}">${escapeHtml(info.clientEmail)}</a></p>` : ''}
  ${info.eventNames ? `<p style="margin:0 0 18px;font-size:14px;color:#555">✒️ Prénoms (mise en page) : ${escapeHtml(info.eventNames)}</p>` : ''}
  ${rows.map((r) => `
