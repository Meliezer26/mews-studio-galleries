...
function buildClientAccessContent(info) {
  ...
  lines.push('');
  lines.push('Veuillez cliquer sur le lien privé ci-dessous.');
  lines.push(gUrl);
  lines.push('');
  lines.push(mdpText);
  lines.push('');
  ...
  const subject = 'Vos photos Mews Studio évènement « ' + galleryName + ' »';
  ...
  const html = `<!doctype html><html><body style="font-family:Arial,Helvetica,sans-serif;color:#222222;line-height:1.6;max-width:600px;margin:0 auto;padding:8px 0">
  ...
  <p style="margin:0 0 18px;font-size:15px">Veuillez cliquer sur le lien privé ci-dessous.</p>
  ${btn}
