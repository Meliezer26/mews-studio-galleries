...
async function sendViaGmail(mailOptions) {
  ...
  let acc = null;
  try { acc = await drive.mailAccount(); } catch { /* identité introuvable */ }
  const accountEmail = (acc && acc.emailAddress) || '';
  if (!accountEmail) throw new Error('Gmail : identité du compte d\'envoi introuvable — reconnectez-le via « Se connecter avec Google (envoi d\'e-mails) ».');
