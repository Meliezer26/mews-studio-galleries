...
function buildClientConfirmationContent(info) {
  ...
  lines.push('');
  if (info.options && info.options.length) {
    lines.push('📦 Options : ' + info.options.map((o) => o.label + ' (\u00d7 ' + o.qty + ')').join(' \u00b7 '));
  }
  lines.push('Total : ' + total + ' photo(s)');
  lines.push('');
  lines.push('Mews Studio va examiner votre sélection et reviendra vers vous.');
  lines.push('Vous pouvez la retrouver dans votre galerie : ' + info.galleryUrl);
  lines.push('');
  lines.push('Envoyé par Mews Studio Galleries.');
  ...
  ${rows.map((r) => `
    ...
    ${r.cover ? `<p style="color:#b57f2a;font-size:13px;margin:0">🖼 Couverture : n°${r.cover.index != null ? r.cover.index : '?'} — ${escapeHtml(r.cover.name)}</p>` : ''}`).join('')}
  ${(info.options && info.options.length) ? `<p style="margin:14px 0 0;padding:10px 12px;background:#faf6ee;border:1px solid #e8dcc0;border-radius:8px;font-size:13px;color:#555">📦 <b>Options :</b> ${info.options.map((o) => escapeHtml(o.label) + ' (\u00d7 ' + o.qty + ')').join(' \u00b7 ')}</p>` : ''}
  <p style="margin-top:22px"><b>Total : ${total} photo(s)</b></p>
  <p style="margin-top:10px;color:#555">Mews Studio va examiner votre sélection et reviendra vers vous.<br>
  Vous pouvez la retrouver dans votre galerie : <a href="${escapeHtml(info.galleryUrl)}">${escapeHtml(info.galleryUrl)}</a></p>
  <p style="color:#999;font-size:12px;margin-top:30px">Envoyé par Mews Studio Galleries.</p>
  </body></html>`;
...
function buildClientAccessContent(info) {
  ...
  <div style="background:#f5f1e8;border:1px solid #e2d9c4;border-radius:10px;padding:18px 18px 16px;margin-bottom:18px;text-align:center">
    <p style="margin:0 0 12px;font-size:14px;color:#5a5347">Pour ouvrir votre galerie :</p>
    <a href="${escapeHtml(gUrl)}" style="display:inline-block;background:#2c2a26;color:#ffffff;text-decoration:none;padding:13px 32px;border-radius:8px;font-weight:600;font-size:15px;">📸 Accéder à ma galerie</a>
  ...
  <p style="margin:0 0 16px">Comment faire :<br>1. Appuyez sur le bouton ci-dessus ;<br>2. Entrez le mot de passe de la galerie ;<br>3. Indiquez votre nom pour retrouver votre sélection à tout moment.</p>
