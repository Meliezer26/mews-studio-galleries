...
/** Notification « dossiers Drive prêts » après le tri automatique. */
async function sendDriveSortNotification(info) {
  if (!isConfigured()) throw new Error('Notifications non configurées.');
  if (!recipient()) throw new Error('Aucun destinataire défini.');
  const rows = Object.values(info.albums || {}).filter((a) => a.added !== undefined);
  const lines = [
    'Bonjour,',
    '',
    'Le tri automatique vient de préparer les dossiers Drive pour la sélection de la galerie « ' + info.galleryName + ' »' + (info.clientName ? ' (' + info.clientName + ')' : '') + '.',
    '',
    ...(rows.length ? rows.map((a) => '▸ ' + a.label + ' : ' + a.added + ' ajoutée(s), ' + (a.removed || 0) + ' retirée(s)') : ['(aucune photo dans cette sélection)']),
    '',
    'Dossier trié : ' + info.folderUrl,
    '',
    'Envoyé par Mews Studio Galleries.',
  ];
  const html = `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#222;line-height:1.55;max-width:640px">
  <h2 style="margin:0 0 6px;font-size:20px">Dossiers Drive prêts ✓</h2>
  <p style="margin:0 0 18px">Le tri automatique a préparé les dossiers pour la sélection de la galerie
  « <b>${escapeHtml(info.galleryName)}</b> »${info.clientName ? ' (' + escapeHtml(info.clientName) + ')' : ''}.</p>
  ${rows.map((r) => `<p style="margin:4px 0;font-size:14px">▸ <b>${escapeHtml(r.label)}</b> : ${r.added} ajoutée(s), ${r.removed || 0} retirée(s)</p>`).join('')}
  <p style="margin-top:22px"><b>Dossier trié :</b> <a href="${escapeHtml(info.folderUrl)}">${escapeHtml(info.folderUrl)}</a></p>
  <p style="color:#999;font-size:12px;margin-top:30px">Envoyé par Mews Studio Galleries.</p>
  </body></html>`;
  await sendMail({
    from: smtp().from,
    to: recipient(),
    subject: 'Dossiers Drive prêts — ' + info.galleryName,
    text: lines.join('\n'),
    html,
  });
}

module.exports = { isConfigured, recipient, sendSelectionNotification, sendDriveSortNotification, sendTest };
