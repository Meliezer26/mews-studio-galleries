 ...
 *  'gmail'  — mode « mon compte Google » actif ET compte connecté
 *  'smtp'   — repli classique (bloqué sur Render gratuit)
 */
function provider() {
  const n = smtp();
  if (n.apiKey) return 'resend';
  if (n.gmailMode && drive.isUserConnected()) return 'gmail';
  return 'smtp';
}
...
function isConfigured() {
  ...
  if (n.gmailMode) return drive.isUserConnected();
  return !!n.host;
}
...
/** Envoi depuis le compte Google du photographe (API Gmail, HTTPS). */
async function sendViaGmail(mailOptions) {
  const n = smtp();
  if (!drive.isUserConnected()) throw new Error('Gmail : compte Google non connecté — reconnectez-le dans la section « Google Drive » (ce qui autorisera aussi l\'envoi d\'e-mails).');
  let acc = null;
  try { acc = await drive.userDriveAccount(); } catch { /* identité introuvable */ }
  const accountEmail = (acc && acc.emailAddress) || '';
  if (!accountEmail) throw new Error('Gmail : identité du compte Google introuvable — reconnectez votre compte dans la section « Google Drive ».');
  const mime = buildMime(mailOptions, fromDisplayName(n.from), accountEmail);
  const raw = Buffer.from(mime, 'utf8').toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
  const res = await drive.userApi('/gmail/v1/users/me/messages/send', {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    if (res.status === 403) {
      throw new Error('Gmail : permission « envoyer des e-mails » refusée — reconnectez votre compte Google (section « Google Drive ») pour autoriser l\'envoi.');
    }
...
async function sendTest() {
  const n = smtp();
  if (n.gmailMode && !drive.isUserConnected()) {
    throw new Error('Gmail : votre compte Google n\'est pas connecté — connectez-le dans la section « Google Drive » de ce même onglet (l\'autorisation inclut l\'envoi d\'e-mails).');
  }
...
function buildClientAccessContent(info) {
  ...
  lines.push('  3. Indiquez votre nom pour retrouver');
  ...
  <div style="background:#f5f1e8;border:1px solid #e2d9c4;border-radius:10px;padding:16px 18px;margin-bottom:18px">
    ...
    ${info.galleryPassword
      ? `<p style="margin:0"><b>Mot de passe de la galerie :</b> ${escapeHtml(info.galleryPassword)}</p>`
      : '<p style="margin:0">Le mot de passe de la galerie vous a déjà été communiqué.</p>'}
  </div>
  <p style="margin:0 0 16px">Comment faire :<br>1. Ouvrez le lien ci-dessus ;<br>2. Entrez le mot de passe de la galerie ;<br>3. Indiquez votre nom pour retrouver votre sélection à tout moment.</p>
...
module.exports = { provider, isConfigured, recipient, sendSelectionNotification, sendDriveFolderNotification, sendDriveAuthWarning, sendTest, sendClientAccessEmail, buildClientAccessContent };
