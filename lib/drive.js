...
const { tokens, saveTokens, tokensMail, saveTokensMail } = require('./store');
...
/** Liste paginée : l'API Drive renvoie au PLUS 1 000 fichiers par appel —
 *  sans suivi du pageToken, un dossier de 1 188 photos n'en renverrait que
 *  1 000. On boucle donc tant qu'un nextToken est renvoyé (plafond sécurité
 *  25 pages). */
async function listAll(requestFn, basePath, label) {
  const out = [];
  let pageToken = '';
  for (let i = 0; i < 25; i++) {
    const res = await requestFn(basePath + (pageToken ? '&pageToken=' + encodeURIComponent(pageToken) : ''));
...
async function listFolders() {
  ...
  return listAll(api, '/drive/v3/files?q=' + q + '&fields=files(id,name),nextPageToken&pageSize=1000&orderBy=name', 'Liste des dossiers Drive impossible');
}
...
/** Liste les sous-dossiers directs d'un dossier Drive (paginé). */
async function listSubfolders(folderId) {
  ...
  return listAll(api, '/drive/v3/files?q=' + q + '&fields=files(id,name),nextPageToken&pageSize=100&orderBy=name', 'Lecture des sous-dossiers impossible');
}
...
async function listImages(folderId) {
  ...
  return listAll(api, '/drive/v3/files?q=' + q + '&fields=files(id,name,mimeType,size,thumbnailLink,createdTime),nextPageToken&pageSize=1000&orderBy=name', 'Lecture du dossier Drive impossible');
}

/** Liste TOUS les fichiers d'un dossier Drive (tous types confondus,
 *  paginé) — diagnostic « le dossier compte 1 188 éléments mais la
 *  galerie n'a que 1 000 photos » : les éléments en plus peuvent être
 *  des vidéos ou documents, invisibles pour la galerie. */
async function listAllFiles(folderId) {
  const q = encodeURIComponent("'" + folderId + "' in parents and trashed=false");
  return listAll(api, '/drive/v3/files?q=' + q + '&fields=files(id,name,mimeType,size),nextPageToken&pageSize=1000&orderBy=name', 'Lecture du dossier Drive impossible');
}

/** Métadonnées d'un dossier (propriétaire, date de création) —
 *  diagnostic « dossier du même nom mais contenu différent » :
 *  révèle si l'application lit un autre dossier/compte que celui
 *  que le photographe regarde dans son Drive. */
async function folderMeta(folderId) {
  const fields = encodeURIComponent('id,name,owners(emailAddress,displayName),sharedWithMeTime,modifiedTime');
  const res = await api(
    '/drive/v3/files/' + encodeURIComponent(folderId) +
    '?fields=' + fields + '&supportsAllDrives=true'
  );
  if (!res.ok) throw new Error('Lecture du dossier impossible (' + res.status + ') — ' + (await res.text()).slice(0, 150));
  return res.json();
}

/** Fetch avec timeout + 1 tentative de rattrapage : une requête Google qui
 *  plante ne bloque plus indéfiniment l'affichage, et un à-coup réseau
 *  transitoire est retenté automatiquement. */
async function fetchResilient(url, headers, timeoutMs = 20000, tries = 2) {
  let lastErr = null;
...
async function fetchThumbnail(thumbnailLink, size = 400) {
  ...
  const token = await accessToken();
  if (!token) throw new Error('GOOGLE_NOT_CONNECTED');
  return fetchResilient(url, { Authorization: 'Bearer ' + token });
}
...
async function fetchMedia(fileId) {
  ...
  return fetchResilient(url, { Authorization: 'Bearer ' + token }, 30000, 2);
}
...
/* ============================================================
 *  OAuth 2e connexion : compte d'ENVOI d'e-mails (API Gmail).
 *  Totalement indépendant de la connexion Drive ci-dessus : le
 *  photographe peut faire trier ses sélections sur le Drive d'un
 *  compte (ex. smelki) et faire partir les e-mails d'un autre
 *  compte (ex. un Gmail pro). Jeton stocké dans tokens-mail.json.
 * ============================================================ */

const MAIL_SCOPES = 'https://www.googleapis.com/auth/gmail.send';

/** Base officielle de l'API Gmail (distincte de la base Drive). */
function gmailApiBase() { return process.env.GOOGLE_GMAIL_BASE || 'https://gmail.googleapis.com'; }

function mailAuthUrl(state) {
  const params = new URLSearchParams({
    client_id: process.env.GOOGLE_CLIENT_ID,
    redirect_uri: redirectUri(),
    response_type: 'code',
...
async function exchangeMailCode(code) {
  const res = await fetch(tokenUrl(), {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error('Échange du code OAuth (envoi e-mails) refusé (' + res.status + ') : ' + text.slice(0, 200));
  }
  return res.json();
}

async function mailRefreshToken() {
  const t = tokensMail();
  if (!t || !t.refresh_token) return null;
  const res = await fetch(tokenUrl(), {
    method: 'POST',
  ...
  if (!res.ok) {
    // Conserver la raison exacte donnée par Google (invalid_grant « Token has
    // been expired or revoked », etc.) pour le diagnostic dans l'admin.
    let body = {};
    ...
    t.lastRefreshError = {
      ...
      description: String(body.error_description || '').slice(0, 200),
    };
    saveTokensMail(t);
    console.warn('[mail] Refresh Gmail refusé :', res.status, body.error, body.error_description);
    return null;
  }
  const data = await res.json();
  t.access_token = data.access_token;
...
async function mailAccessToken() {
  const t = tokensMail();
  if (!t || !t.refresh_token) return null;
  if (t.access_token && t.expiry && Date.now() < t.expiry - 60 * 1000) return t.access_token;
...
/** Appel API authentifié avec le compte d'envoi uniquement (auto-refresh). */
async function mailApi(path, init = {}) {
  const token = await mailAccessToken();
  if (!token) throw new Error('GOOGLE_MAIL_NOT_CONNECTED');
  let res = await fetch(gmailApiBase() + path, {
    ...init,
    headers: { Authorization: 'Bearer ' + token, ...(init.headers || {}) },
  });
  if (res.status === 401) {
    const refreshed = await mailRefreshToken();
    if (refreshed) {
      res = await fetch(gmailApiBase() + path, {
        ...init,
        headers: { Authorization: 'Bearer ' + refreshed, ...(init.headers || {}) },
      });
    }
...
/** Adresse du compte d'envoi connecté (cache locale d'abord, API en secours). */
async function mailAccount() {
  const t = tokensMail();
  if (t && t.email) return { emailAddress: t.email };
  try {
    const res = await mailApi('/gmail/v1/users/me/profile?fields=emailAddress');
    if (!res.ok) return { emailAddress: null, error: 'HTTP ' + res.status };
    const data = await res.json();
    if (data.emailAddress && t) { t.email = data.emailAddress; saveTokensMail(t); }
    return { emailAddress: data.emailAddress || null };
  } catch (e) {
    return { emailAddress: null, error: String(e.message || e).slice(0, 120) };
  }
}

/** Liste les dossiers visibles par le compte utilisateur (paginé). */
async function listUserFolders() {
  const q = encodeURIComponent("mimeType='application/vnd.google-apps.folder' and trashed=false");
  return listAll(userApi, '/drive/v3/files?q=' + q + '&fields=files(id,name),nextPageToken&pageSize=1000&orderBy=name', 'Liste des dossiers impossible');
}
...
/** Crée un fichier texte dans un dossier (compte utilisateur).
 *  Upload MULTIPART (même mécanisme que uploadToFolder, éprouvé en prod) —
 *  l'upload « media » simple renvoyait une 400 inexplicable côté Drive. */
async function createUserTextFile(name, parentId, content) {
  const boundary = 'mews' + Date.now().toString(36);
  ...
  const res = await userApi('/upload/drive/v3/files?uploadType=multipart&fields=id,name', {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error('Création du fichier « ' + name + ' » refusée (' + res.status + (errBody ? ' : ' + errBody.slice(0, 200) : '') + ')');
  }
  return res.json();
}
...
module.exports = {
  ...
  driveAccount,
  userDriveAccount,
  // Compte d'envoi d'e-mails (indépendant du compte Drive)
  mailAuthUrl,
  exchangeMailCode,
  mailRefreshToken,
  mailAccessToken,
