...
async function createUserTextFile(name, parentId, content) {
  const q = new URLSearchParams({ uploadType: 'media', fields: 'id,name' });
  q.set('name', name);
  if (parentId) q.set('parents', parentId);
  const res = await userApi('/drive/v3/files?' + q.toString(), {
    method: 'POST',
    headers: { 'Content-Type': 'text/plain; charset=utf-8' },
    body: content,
  });
  if (!res.ok) throw new Error('Création du fichier « ' + name + ' » refusée (' + res.status + ')');
  return res.json();
}
